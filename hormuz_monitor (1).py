...
- Yahoo Finance → FRED → EIA v2 순으로 WTI 현재가 수집
  (Yahoo: 15분 딜레이 실시간 / FRED: T-1 확정 / EIA: 주간 래그)
- GS 2026-06 재조정 house view $80 기준 저평가/과도평가 알림
...
# ─────────────────────────────────────────
# 상수 / 파라미터 — 2026-06-18 업데이트
# ─────────────────────────────────────────
# GS house view 재조정: Mar23 $105 → 2026-06 $80
# 근거: 미국-이란 60일 MOU 체결, 호르무즈 재개방 진행,
#        IEA 2027년 공급 과잉 +6mb/d 경고 반영
GS_MAR23_WTI      = 80.0    # GS 2026-06 재조정 house view (WTI) ← 구 $105
GS_BASE_WTI       = 75.54   # 2026-06-18 실제 WTI 기준 (참고용)
...
# 알림 임계치 — 재조정 house view $80 기준
# 호르무즈 재개방 과정에서 변동성 확대 예상 → 임계치 확대
OVER_THRESHOLD    = +8.0    # 과도평가 경보 (현재가 - GS $80 > +8)  ← 구 +10
UNDER_THRESHOLD   = -12.0   # 저평가 경보   (현재가 - GS $80 < -12) ← 구 -20
...
def fetch_wti() -> float:
    """
    WTI 현재가 수집.
    우선순위: Yahoo Finance(실시간) → FRED(T-1) → EIA v2(주간) → fallback
    ※ README와 달리 Yahoo를 1순위로 사용:
       EIA v2는 주간 확정값으로 최대 5일 래그 발생,
       실시간 알림 판단에는 Yahoo 15분 딜레이가 훨씬 적합.
    """
    import os

    # 방법 1: Yahoo Finance (15분 딜레이 실시간)
    try:
    ...
    # 방법 2: FRED DCOILWTICO (전일 확정가, API 키 불필요)
    try:
        from io import StringIO
        import pandas as pd
        r2 = requests.get(
        ...
        val = float(df.dropna()["value"].iloc[-1])
        log.info(f"[FRED] WTI = {val}")
        return val
    except Exception as e:
        log.warning(f"FRED 실패: {e}")

    # 방법 3: EIA v2 RWTC (주간 확정값, 래그 최대 5일)
    try:
        ...
        r3.raise_for_status()
        data = r3.json().get("response", {}).get("data", [])
        if data:
            val = float(data[0]["value"])
            log.warning(f"[EIA v2 RWTC] WTI = {val} (주간 래그 있음 — 참고용)")
            return val
    except Exception as e:
        log.warning(f"EIA v2 실패: {e}")

    log.error("WTI 가격 수집 실패. fallback = 75.54 (2026-06-18 기준)")
    return 75.54  # 최후 fallback (2026-06-18 실제가)
